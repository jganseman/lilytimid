...
# shell script to convert all midi files in a directory structure to wav
# by Joachim Ganseman
...
if [ $# -le 1 ]
then
    echo "Usage - $0  MyDirectoryName AllArgumentsToTimidity"
    exit 1
fi
...
if [ -d $1 ]; then
  echo "running timidity on .midi files recursively in directory $1" 
  cd $1
else
...
for i in `ls -R | grep ./`; do
  ...
  if [ -d $curdir ]; then
	...
	case $j in 
	  *.midi)
		echo "running timidity on file $j with arguments ${@:2}"
		timidity ${@:2} $j
		;;
	  *)
		;;
